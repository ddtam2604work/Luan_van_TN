...
class AnalysisModule:
    ...
    def analyze(self, ingestion_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"[GĐ 3] Đang phân tích rủi ro: {ingestion_data.get('filename', 'Unknown')}")
        ...
        # ==========================================================
        # [ĐÃ ĐIỀU CHỈNH] Thu thập has_hash & Gom logic kill-switch mã độc
        # ==========================================================
        has_hash = bool(ingestion_data.get('hash'))
        malware_found = (malicious_vt > 0) or (yara_alerts > 0)

        # [ĐÃ ĐIỀU CHỈNH] Truyền đủ 3 tham số cho Luồng CI
        base_ci = self.engine.map_integrity(has_sig=has_sig, has_hash=has_hash, malware_found=malware_found)
        
        # [ĐÃ ĐIỀU CHỈNH] Bỏ map_malware, gán base_cm = 10.0 để không ảnh hưởng 
        # đến việc tính điểm CM từ Scorecard của từng package bên dưới
        base_cm = 10.0
        ...
        for comp in sbom.get('components', []):
            ...
            # Ghi chú nghiệp vụ: Tạm thời CM vẫn lấy theo Scorecard (m_score).
            # Khi bạn cập nhật API kéo data GitHub/PyPI, hãy đổi chỗ này thành self.engine.map_maintenance(...)
